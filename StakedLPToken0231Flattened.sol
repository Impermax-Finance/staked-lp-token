...
// File: contracts\interfaces\IMasterChef0231.sol
...
// File: contracts\StakedLPToken0231.sol

pragma solidity =0.5.16;
contract StakedLPToken0231 is IUniswapV2Pair, PoolToken {
	using SafeToken for address;
	
	bool public constant isStakedLPToken = true;
	string public constant stakedLPTokenType = "0231";
	...
	function _update() internal {
		(uint256 _totalBalance,) = IMasterChef0231(masterChef).userInfo(pid, address(this));
	...
	function mint(address minter) external nonReentrant update returns (uint mintTokens) {
		...
		(uint256 _totalBalanceBefore,) = IMasterChef0231(masterChef).userInfo(pid, address(this));
		IMasterChef0231(masterChef).deposit(pid, mintAmount);
		(uint256 _totalBalanceAfter,) = IMasterChef0231(masterChef).userInfo(pid, address(this));
	...
	function redeem(address redeemer) external nonReentrant update returns (uint redeemAmount) {
		...
		_burn(address(this), redeemTokens);
		IMasterChef0231(masterChef).withdraw(pid, redeemAmount);
		_safeTransfer(redeemer, redeemAmount);
		emit Redeem(msg.sender, redeemer, redeemAmount, redeemTokens);		
	}
	...
	function _getReward() internal returns (uint256) {
		IMasterChef0231(masterChef).withdraw(pid, 0);
		(,,,,,address rewarder) = IMasterChef0231(masterChef).poolInfo(pid);
		if (rewarder != address(0)) {
			...
			if (myBalance > 0 && token != rewardsToken) {
				// Convert token to rewardsToken
				if (token != WETH) swapExactTokensForTokens(token, WETH, myBalance);
	...
	function reinvest() external nonReentrant update {
		...
		IMasterChef0231(masterChef).deposit(pid, liquidity);
...
// File: contracts\StakedLPTokenFactory0231.sol

pragma solidity =0.5.16;
contract StakedLPTokenFactory0231 {
	address public router;
	address public masterChef;
	address public rewardsToken;
	address public WETH;
	...
	function createStakedLPToken(uint256 pid) external returns (address payable stakedLPToken) {
		...
		(address pair,,,,,) = IMasterChef0231(masterChef).poolInfo(pid);
		address token0 = IUniswapV2Pair(pair).token0();
		address token1 = IUniswapV2Pair(pair).token1();
		bytes memory bytecode = type(StakedLPToken0231).creationCode;
		assembly {
			stakedLPToken := create2(0, add(bytecode, 32), mload(bytecode), pid)
		}
		StakedLPToken0231(stakedLPToken)._initialize(pid, pair, token0, token1, router, masterChef, rewardsToken, WETH);
