...
import "./interfaces/ISolidlyGauge8.sol";
import "./libraries/SafeToken.sol";
import "./libraries/Math.sol";

// updated codebase for thena v2 and pearl

contract StakedLPTokenSolidlyBase8 is PoolToken {
	using SafeToken for address;
	
	bool public constant isStakedLPToken = true;
	string public constant stakedLPTokenType = "SolidlyBase8";
	...
	function _update() internal {
		uint256 _totalBalance = ISolidlyGauge8(gauge).balanceOf(address(this));
	...
	function mint(address minter) external nonReentrant update returns (uint mintTokens) {
		...
		uint256 _totalBalanceBefore = ISolidlyGauge8(gauge).balanceOf(address(this));
		ISolidlyGauge8(gauge).deposit(mintAmount,0);
		uint256 _totalBalanceAfter = ISolidlyGauge8(gauge).balanceOf(address(this));
	...
	function redeem(address redeemer) external nonReentrant update returns (uint redeemAmount) {
		...
		ISolidlyGauge8(gauge).withdraw(redeemAmount);
	...
	function _getReward() internal returns (uint256) {
		...
		ISolidlyGauge8(gauge).getReward(address(this), tokens);
	...
	function reinvest() external nonReentrant update {
		...
		ISolidlyGauge8(gauge).deposit(liquidity,0);
