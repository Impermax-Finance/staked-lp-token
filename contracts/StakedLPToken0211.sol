...
import "./interfaces/IStakedLPToken02.sol";
import "./interfaces/IMasterChef0211.sol";
...
contract StakedLPToken0211 is IStakedLPToken02, IUniswapV2Pair, PoolToken {
	using SafeToken for address;
	
	bool public constant isStakedLPToken = true;
	string public constant stakedLPTokenType = "0211";
	...
	function _update() internal {
		(uint256 _totalBalance,) = IMasterChef0211(masterChef).userInfo(pid, address(this));
	...
	function mint(address minter) external nonReentrant update returns (uint mintTokens) {
		...
		(uint256 _totalBalanceBefore,) = IMasterChef0211(masterChef).userInfo(pid, address(this));
		IMasterChef0211(masterChef).deposit(pid, mintAmount, address(this));
		(uint256 _totalBalanceAfter,) = IMasterChef0211(masterChef).userInfo(pid, address(this));
	...
	function redeem(address redeemer) external nonReentrant update returns (uint redeemAmount) {
		...
		IMasterChef0211(masterChef).withdraw(pid, redeemAmount, address(this));
	...
	function _getReward() internal returns (uint256) {
		IMasterChef0211(masterChef).harvest(pid, address(this));
	...
	function reinvest() external nonReentrant update {
		...
		IMasterChef0211(masterChef).deposit(pid, liquidity, address(this));
