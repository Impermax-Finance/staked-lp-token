...
// Samve as blackhole1 but we removed reward token from deployment

contract StakedLPTokenSolidlyBlackhole2 is PoolToken {
	using SafeToken for address;
	
	bool public constant isStakedLPToken = true;
	string public constant stakedLPTokenType = "solidly10";
    ...
    address pair = IRouter(router).pairFor(tokenIn, tokenOut, false);
      ...
      receiver: address(this) 
		...
		IRouter(router).swapExactTokensForTokens(amount, 0, route, address(this), now);
	}
	...
	function addLiquidity(address tokenA, address tokenB, uint256 amountA, uint256 amountB) internal returns (uint256 liquidity) {
		...
		(,,liquidity) = IRouter(router).addLiquidity(tokenA, tokenB, false, amountA, amountB, 0, 0, address(this), now);
	}
	...
	function _swapWithBestBridge() internal view returns (address bestBridgeToken, uint bestIndex) {
		...
		address pairFactory = IRouter(router).factory();
		for (uint i = 0; i < bridgeTokens.length; i++) {
			for (uint j = 0; j < 2; j++) {
