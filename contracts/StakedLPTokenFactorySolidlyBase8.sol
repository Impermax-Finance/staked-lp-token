...
import "./StakedLPTokenSolidlyBase8.sol";
import "./interfaces/IUniswapV2Pair.sol";
import "./interfaces/IUniswapV2Router01.sol";

// updated codebase for thena v2 and pearl

contract StakedLPTokenFactorySolidlyBase8 {
	...
	function createStakedLPToken(address pair) external returns (address stakedLPToken) {
		...
		bytes memory bytecode = type(StakedLPTokenSolidlyBase8).creationCode;
		assembly {
			stakedLPToken := create2(0, add(bytecode, 32), mload(bytecode), pair)
		}
		StakedLPTokenSolidlyBase8(stakedLPToken)._initialize(pair, token0, token1, router, voter, rewardsToken, bridgeTokens);
