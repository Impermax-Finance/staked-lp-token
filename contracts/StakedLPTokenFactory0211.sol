...
import "./StakedLPToken0211.sol";
import "./interfaces/IMasterChef0211.sol";
import "./interfaces/IStakedLPTokenFactory02.sol";
import "./interfaces/IUniswapV2Pair.sol";
import "./interfaces/IUniswapV2Router01.sol";

contract StakedLPTokenFactory0211 is IStakedLPTokenFactory02 {
	...
	function createStakedLPToken(uint256 pid) external returns (address stakedLPToken) {
		...
		address pair = IMasterChef0211(masterChef).lpToken(pid);
		address token0 = IUniswapV2Pair(pair).token0();
		address token1 = IUniswapV2Pair(pair).token1();
		bytes memory bytecode = type(StakedLPToken0211).creationCode;
		assembly {
			stakedLPToken := create2(0, add(bytecode, 32), mload(bytecode), pid)
		}
		StakedLPToken0211(stakedLPToken)._initialize(pid, pair, token0, token1, router, masterChef, rewardsToken, WETH);
