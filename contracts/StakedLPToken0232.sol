...
contract StakedLPToken0232 is IUniswapV2Pair, PoolToken {
    using SafeToken for address;

    bool public constant isStakedLPToken = true;
    string public constant stakedLPTokenType = "0232";
    ...
    function _update() internal {
        uint256 _totalBalance = IMasterChef0232(masterChef).getDeposit(
            pid,
            address(this)
        );
    ...
    function mint(
        ...
        uint256 _totalBalanceBefore = IMasterChef0232(masterChef).getDeposit(
            pid,
            address(this)
        );
        IMasterChef0232(masterChef).deposit(pid, mintAmount);
        uint256 _totalBalanceAfter = IMasterChef0232(masterChef).getDeposit(
            pid,
            address(this)
        );
    ...
    function redeem(
        ...
        require(
            ...
            "StakedLPToken: INSUFFICIENT_CASH"
        );
        _burn(address(this), redeemTokens);
        IMasterChef0232(masterChef).withdraw(pid, redeemAmount);
    ...
    function _getReward() internal returns (uint256) {
        ...
        pids[0] = pid;
        IMasterChef0232(masterChef).claim(pids);

        address extraRewarder = IMasterChef0232(masterChef).getExtraRewarder(
            pid
        );
        if (extraRewarder != address(0)) {
            (address token, , , ) = IExtraRewarder(extraRewarder)
            ...
            if (token != address(0)) {
                ...
                if (myBalance > 0 && token != rewardsToken) {
                    ...
                    if (token != WETH)
                        swapExactTokensForTokens(token, WETH, myBalance);
    ...
    function reinvest() external nonReentrant update {
        ...
        IMasterChef0232(masterChef).deposit(pid, liquidity);
        ...
        returns (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast)
    {
        ...
        require(
            ...
            "StakedLPToken: INSUFFICIENT_RESERVES"
        );
    }
