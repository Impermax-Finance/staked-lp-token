...
contract StakedLPTokenFactory0221 {
	...
	constructor(address _router, address _masterChef, address _rewardsToken, address[] memory _bridgeTokens) public {
		router = _router;
		masterChef = _masterChef;
		rewardsToken = _rewardsToken;
		bridgeTokens = _bridgeTokens;
	...
	function createStakedLPToken(uint256 pid) external returns (address stakedLPToken) {
		...
		StakedLPToken0221(stakedLPToken)._initialize(pid, pair, token0, token1, router, masterChef, rewardsToken, bridgeTokens);
