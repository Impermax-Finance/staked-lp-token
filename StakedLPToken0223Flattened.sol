...
// File: contracts\interfaces\IMasterChef0223.sol
...
// File: contracts\StakedLPToken0223.sol

pragma solidity =0.5.16;
contract StakedLPToken0223 is IUniswapV2Pair, PoolToken {
	using SafeToken for address;
	
	bool public constant isStakedLPToken = true;
	string public constant stakedLPTokenType = "0223";
	...
	function _initialize(
		...
		_rewardsToken1.safeApprove(address(_router), uint256(-1));
	...
	function _update() internal {
		(uint256 _totalBalance,,) = IMasterChef0223(masterChef).userInfo(pid, address(this));
	...
	function mint(address minter) external nonReentrant update returns (uint mintTokens) {
		...
		(uint256 _totalBalanceBefore,,) = IMasterChef0223(masterChef).userInfo(pid, address(this));
		IMasterChef0223(masterChef).deposit(pid, mintAmount);
		(uint256 _totalBalanceAfter,,) = IMasterChef0223(masterChef).userInfo(pid, address(this));
	...
	function redeem(address redeemer) external nonReentrant update returns (uint redeemAmount) {
		...
		_burn(address(this), redeemTokens);
		IMasterChef0223(masterChef).withdraw(pid, redeemAmount);
		_safeTransfer(redeemer, redeemAmount);
		emit Redeem(msg.sender, redeemer, redeemAmount, redeemTokens);		
	}
	...
	function _getReward() internal returns (uint256) {
		IMasterChef0223(masterChef).withdraw(pid, 0);
		uint256 rewardsToken1Balance = rewardsToken1.myBalance();
		if (rewardsToken1Balance > 0) {
			// rewardsToken1 is WETH, we have to swap to USDC and then to ARX
			address bridgeToken = bridgeTokens[0];
			swapExactTokensForTokens(rewardsToken1, bridgeToken, rewardsToken1Balance);
			swapExactTokensForTokens(bridgeToken, rewardsToken, bridgeToken.myBalance());
		}
	...
	function _swapWithBestBridge() internal view returns (address) {
		...
		for (uint i = 0; i < bridgeTokens.length; i++) {	
			address pair = IUniswapV2Factory(factory).getPair(token0, bridgeTokens[i]);
	...
	function reinvest() external nonReentrant update {
		...
		IMasterChef0223(masterChef).deposit(pid, liquidity);
...
// File: contracts\StakedLPTokenFactory0223.sol
