...
// File: contracts\interfaces\ILpDepositor.sol
...
// File: contracts\StakedLPToken09.sol

pragma solidity =0.5.16;

contract StakedLPToken09 is PoolToken {
	using SafeToken for address;
	
	bool public constant isStakedLPToken = true;
	string public constant stakedLPTokenType = "09";
	...
	function _update() internal {
		uint256 _totalBalance = ILpDepositor(lpDepositor).userBalances(address(this), underlying);
	...
	function mint(address minter) external nonReentrant update returns (uint mintTokens) {
		...
		uint256 _totalBalanceBefore = ILpDepositor(lpDepositor).userBalances(address(this), underlying);
		ILpDepositor(lpDepositor).deposit(underlying, mintAmount);
		uint256 _totalBalanceAfter = ILpDepositor(lpDepositor).userBalances(address(this), underlying);
	...
	function redeem(address redeemer) external nonReentrant update returns (uint redeemAmount) {
		...
		_burn(address(this), redeemTokens);
		ILpDepositor(lpDepositor).withdraw(underlying, redeemAmount);
		_safeTransfer(redeemer, redeemAmount);
		emit Redeem(msg.sender, redeemer, redeemAmount, redeemTokens);		
	}
	...
	function _getReward() internal returns (uint256) {
		...
		ILpDepositor(lpDepositor).getReward(pools);
	...
	function reinvest() external nonReentrant update {
		...
		ILpDepositor(lpDepositor).deposit(underlying, liquidity);
...
// File: contracts\StakedLPTokenFactory09.sol
